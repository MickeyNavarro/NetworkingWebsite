...
class UsersJobPostingsDataService{
    ...
    function readByUserID($id){
        try {
            Log::info("Entering UsersJobPostingsDataService.readByUserID()");
            
            //use the connection to create a prepared statement
            $stmt = $this->conn->prepare("SELECT * FROM `USERS_JOB_POSTINGS` WHERE USERS_ID = :id");
            ...
            if($stmt->rowCount() == 0){
                Log::info("Exiting UsersJobPostingsDataService.readByUserID() with returning null");
                return null;
            }
            else{
                ...
                Log::info("Exiting UsersJobPostingsDataService.readByUserID() with returning a skill object");
                return $uj_arr;            
            }
        }
    ...
    function delete($id){
        try {
            ...
            //use the connection to create a prepared statement
            $stmt = $this->conn->prepare("DELETE FROM `USERS_JOB_POSTINGS` WHERE `USERS_ID` = :id");
